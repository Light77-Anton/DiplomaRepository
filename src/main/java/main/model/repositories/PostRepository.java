...
public interface PostRepository extends JpaRepository<Post, Integer> {
    ...
    Page<CountForPostId> findAllAndOrderByCommentariesSize(Pageable pageable); // под вопросом

    @Query(value = "SELECT new CountForPostId(p.id, COUNT(pv.id)) FROM posts AS p "
            + "LEFT JOIN post_votes AS pv ON pv.post_id = p.id AND pv.value = 1 "
            + "WHERE p.is_active = true AND p.moderation_status = 'ACCEPTED' AND p.time > now() "
            + "GROUP BY p.id ORDER BY COUNT(pv.id) desc", nativeQuery = true)
    Page<CountForPostId> findAllAndOrderByVotesCount(Pageable pageable); // под вопросом

    @Query(value = "SELECT * FROM posts AS p WHERE p.is_active = true AND p.time > now() ORDER BY p.time DESC", nativeQuery = true)
